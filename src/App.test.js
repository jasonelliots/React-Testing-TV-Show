...
test("successfully renders data from api - show title", async () => {
  mockFetchShow.mockResolvedValueOnce(showData);

  const { getByText, getByTestId } = render(<App />);

  await waitFor(() => {
    const header = getByTestId(/title/i);
    expect(header).toBeInTheDocument();
  });
});

test("successfully renders data from api - episodes", async () => {
  mockFetchShow.mockResolvedValueOnce(showData);
  ...
  waitFor(() => {
    ...
    expect(getAllByTestId(/show/i)).toHaveLength(3);
  });
});
